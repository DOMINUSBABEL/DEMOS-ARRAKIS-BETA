    ...
    return (
        <div className="relative" ref={menuRef}>
            <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg transition-all duration-300 hover:shadow-[0_0_15px_rgba(34,197,94,0.4)] flex items-center gap-2"
            >
                Exportar Informe
                <svg className={`w-4 h-4 transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
            </button>
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-light-card dark:bg-dark-card rounded-md shadow-lg z-20 border border-light-border dark:border-dark-border animate-fade-in-up">
    ...
    switch (activeTab) {
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6" id="analysis-charts">
                        ...
                        <AnalysisCard title="Análisis del Estratega IA" explanation="Obtén un análisis estratégico instantáneo basado en el Ranking de Poder Electoral Base. La IA identificará fortalezas, debilidades y el panorama competitivo inicial, enriquecido con datos de Google Search." collapsible>
                            <button onClick={() => handleAiAnalysis('general')} disabled={isAiLoading['general']} className="w-full flex items-center justify-center gap-2 bg-brand-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-brand-secondary transition-all duration-300 hover:shadow-[0_0_15px_rgba(217,119,6,0.4)] disabled:opacity-50 disabled:cursor-wait">
                                <SparklesIcon className="w-5 h-5" />
                                Analizar Ranking con IA
                            </button>
                            {isAiLoading['general'] && (
                                <div className="mt-4 text-center py-8 bg-dark-bg/30 rounded-lg animate-pulse-slow">
                            ...
                            {!isAiLoading['general'] && aiAnalysis['general']?.text && (
                                <>
                                    <pre id="ai-analysis-text" className="whitespace-pre-wrap font-sans text-sm max-w-none mt-4 bg-light-bg dark:bg-dark-bg p-4 rounded-md border border-light-border dark:border-dark-border text-light-text-primary dark:text-dark-text-primary animate-fade-in">
                                        {aiAnalysis['general'].text}
                                    </pre>
                                    {aiAnalysis['general']?.sources && aiAnalysis['general'].sources.length > 0 && (
                                        <div className="mt-4 p-4 rounded-md bg-dark-bg/50 border border-dark-border animate-fade-in">
      ...
      case 'projections':
        return activeDataset && <div className="space-y-6">
            ...
            {isSimulating && (
              <div className="text-center py-10 bg-dark-card rounded-lg animate-pulse">
                <LoadingSpinner className="w-8 h-8 mx-auto text-brand-primary" />
                ...
                <div className="space-y-6 animate-fade-in-up">
                  <AnalysisCard title="Análisis IA de la Simulación" explanation={`Obtén un análisis estratégico de los resultados de la simulación de ${analysisSubjectPlural.toLowerCase()}.`} collapsible defaultCollapsed>
                    ...
                    {isAiLoading['projections'] && (
                        <div className="mt-4 text-center py-8 bg-dark-bg/30 rounded-lg animate-pulse">
                            <LoadingSpinner className="w-8 h-8 mx-auto text-brand-primary" />
                    ...
                    {!isAiLoading['projections'] && aiAnalysis['projections']?.text && (
                        <>
                            <pre className="whitespace-pre-wrap font-sans text-sm max-w-none mt-4 bg-dark-bg p-4 rounded-md border border-dark-border animate-fade-in">
